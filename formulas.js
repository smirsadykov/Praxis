...
window.FORMULAS = {
  ...
  "quadratic-equations": [
    { f: "$x = \\dfrac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}$", n: "QUADRATIC FORMULA. ORIGIN: Babylonian mathematicians solved specific quadratics around 2000 BCE. Brahmagupta (628 CE) wrote down a near-modern version. The complete formula in current notation comes from Renaissance algebraists (Cardano, Viète, ~1500s). HOW IT'S DERIVED: take $ax^2 + bx + c = 0$, divide by $a$, complete the square on $x^2 + (b/a)x$, isolate $x$. Falls out in 4 lines. WORKED EXAMPLE: solve $2x^2 + 3x - 2 = 0$. Plug in $a = 2, b = 3, c = -2$: $x = \\dfrac{-3 \\pm \\sqrt{9 + 16}}{4} = \\dfrac{-3 \\pm 5}{4}$. So $x = 1/2$ or $x = -2$. The formula handles ANY quadratic — that's why it's worth memorizing." },
    { f: "$\\Delta = b^2 - 4ac$", n: "DISCRIMINANT — the part under the square root in the quadratic formula. ORIGIN: the name 'discriminant' comes from Latin 'to distinguish' — because $\\Delta$ DISTINGUISHES the type of solutions. $\\Delta > 0$: TWO distinct real roots (parabola crosses x-axis twice). $\\Delta = 0$: ONE repeated root (parabola just touches x-axis). $\\Delta < 0$: NO real roots (parabola never crosses the axis — the roots are complex conjugates, which you'll meet later). WORKED EXAMPLE: $x^2 + 2x + 5 = 0$ has $\\Delta = 4 - 20 = -16 < 0$. No real roots; this parabola hovers above the x-axis." },
    { f: "$x_1 + x_2 = -\\dfrac{b}{a}, \\; x_1 x_2 = \\dfrac{c}{a}$", n: "VIETA'S FORMULAS (François Viète, ~1591). ORIGIN: Viète realized that the SUM and PRODUCT of the roots can be read directly off the coefficients. HOW IT'S DERIVED: if $x_1$ and $x_2$ are the roots, then $a(x - x_1)(x - x_2) = ax^2 + bx + c$. Expand the left side and match coefficients. WORKED EXAMPLE: for $x^2 - 5x + 6 = 0$, sum of roots = $5$, product = $6$. So the roots are $2$ and $3$ (whose sum is $5$, product is $6$) — solved without the quadratic formula. Useful for quickly checking factorizations." },
    { f: "Vertex $\\left(-\\dfrac{b}{2a}, \\; c - \\dfrac{b^2}{4a}\\right)$", n: "The VERTEX of a parabola — its lowest point if $a > 0$ (opens up) or highest if $a < 0$ (opens down). ORIGIN: the parabola is symmetric about a vertical line through the vertex, and that line of symmetry must be at $x = -b/(2a)$ (the average of the roots, by Vieta's). HOW TO FIND IT: take the derivative, set to zero — $2ax + b = 0$ gives $x = -b/(2a)$. WORKED EXAMPLE: a ball's height is $h(t) = -5t^2 + 20t + 2$. Maximum height occurs at $t = -20/(2 \\cdot -5) = 2$ s. Maximum value: $h(2) = -5(4) + 20(2) + 2 = 22$ m. Used in optimization problems all the time." },
    { f: "Complete the square: $x^2 + bx = (x + \\tfrac{b}{2})^2 - \\tfrac{b^2}{4}$", n: "COMPLETING THE SQUARE — the algebraic identity that makes the quadratic formula derivable. ORIGIN: ancient Babylonian and Greek mathematicians used it geometrically (literally completing a square shape on a diagram). HOW IT WORKS: take $x^2 + bx$, add and subtract $(b/2)^2$ to make the first three terms a perfect square. Magical move that converts a quadratic into 'squared thing equals constant.' WORKED EXAMPLE: solve $x^2 + 6x - 7 = 0$. Add 9 to both sides: $x^2 + 6x + 9 = 16$, i.e. $(x+3)^2 = 16$. Take square root: $x + 3 = \\pm 4$, so $x = 1$ or $x = -7$. The technique appears throughout calculus (for evaluating integrals) and differential equations." }
  ],
  ...
  "newtons-laws": [
    { f: "$\\vec F_{\\text{net}} = m \\vec a$", n: "Newton's second law — published in his *Principia* in 1687. ORIGIN: Galileo (early 1600s) had noticed that objects fall with the same acceleration regardless of mass — heavy ones don't fall faster, contrary to Aristotle. Newton generalized this insight: the same acceleration-from-force relationship governs ALL motion. HOW HE GOT IT: by carefully analyzing the orbit of the Moon and apple falling, Newton realized force was proportional to BOTH mass and acceleration (not just velocity, as Aristotle thought). WORKED EXAMPLE: a $5$ kg box on a frictionless floor is pushed with $20$ N. What's $a$? Plug in: $a = F/m = 20/5 = 4$ m/s². Double the force or halve the mass to double the acceleration. This single equation built the Industrial Revolution." },
    { f: "$\\vec F_{12} = -\\vec F_{21}$", n: "Newton's third law (also *Principia*, 1687). ORIGIN: Newton noticed that pushing on a wall, the wall pushes back. Pulling on a rope, the rope pulls back. Every interaction has a partner force, equal in magnitude and opposite in direction. WORKED EXAMPLE: when you walk forward, your foot pushes the ground BACKWARD. The ground (by Newton 3) pushes you FORWARD. That forward push is what propels you. Without Newton 3, walking would be impossible. WHY IT FOOLS PEOPLE: 'they cancel out' — NO, the pair acts on DIFFERENT objects (your foot and the ground), so neither object feels both halves of the pair. The two forces only 'cancel' from the system's center-of-mass perspective." },
    { f: "$W = mg$", n: "WEIGHT = mass × gravitational acceleration. ORIGIN: a special case of $F = ma$ where the force is gravity (with $g \\approx 9.8$ m/s² near Earth's surface, discovered by Galileo via inclined-plane experiments). DISTINCTION: WEIGHT is a force, measured in newtons; MASS is intrinsic, measured in kilograms. Common confusion: in everyday English, 'weight' colloquially means mass. WORKED EXAMPLE: your mass is $70$ kg. Weight on Earth: $70 \\times 9.8 = 686$ N. Weight on the Moon (where $g \\approx 1.6$ m/s²): $70 \\times 1.6 = 112$ N. Same mass, different weight. In freefall (orbiting astronaut), you're WEIGHTLESS but not massless." },
    { f: "$f_s \\leq \\mu_s N$", n: "STATIC FRICTION inequality. The friction force on a non-moving object adjusts to prevent motion — UP TO a maximum of $\\mu_s N$ (coefficient of static friction times normal force). Once you push past that limit, the surface starts sliding. HISTORICAL NOTE: Leonardo da Vinci first described friction laws around 1500; Amontons and Coulomb formalized them in the 1700s. WORKED EXAMPLE: a $10$ kg block on a floor with $\\mu_s = 0.4$. Normal force $N = mg = 98$ N. Maximum static friction: $0.4 \\times 98 = 39.2$ N. So you can push horizontally with up to $39$ N and the block stays still — at $40$ N it begins to slide." },
    { f: "$f_k = \\mu_k N$", n: "KINETIC FRICTION — the force opposing motion of a sliding object. Unlike static friction, it has a DEFINITE value (not an inequality). $\\mu_k$ is typically smaller than $\\mu_s$: it's HARDER to start sliding than to keep it sliding. (This is why a stuck door suddenly flies open once you finally overcome static friction.) WORKED EXAMPLE: pushing the same $10$ kg block with $\\mu_k = 0.3$ across the floor. Kinetic friction: $0.3 \\times 98 = 29.4$ N. To slide at constant velocity, push with exactly $29.4$ N. Push with more and the block accelerates (net force $= F_\\text{push} - f_k$)." },
    { f: "Tension transmits force", n: "An ideal (massless, inextensible) ROPE or string transmits force from one end to the other UNCHANGED in magnitude. Tension is uniform throughout the rope. ORIGIN: comes directly from Newton's 2nd law applied to a massless rope (zero mass, so any non-zero net force would imply infinite acceleration — contradiction; so tension at every point must balance, meaning constant tension). WORKED EXAMPLE: you pull on a rope attached to a 5 kg box with 30 N. The box experiences exactly 30 N pulling it — none of the force is 'used up' by the rope itself. Real ropes have small mass, so this is an approximation that works well for light, taut strings." },
    { f: "Normal force ⊥ surface", n: "NORMAL FORCE is the contact force a surface exerts on an object PERPENDICULAR to itself, preventing the object from penetrating. ORIGIN: comes from Newton's 2nd law constraint — the surface must push back hard enough to prevent acceleration into it. The MAGNITUDE adjusts automatically to whatever's needed. WORKED EXAMPLE: a $5$ kg block sits on a flat floor. Gravity pulls down with $49$ N. Normal force from the floor pushes up with EXACTLY $49$ N (so net vertical force = 0, no vertical acceleration). On an inclined plane at angle $\\theta$, only the perpendicular component of gravity matters: $N = mg\\cos\\theta$. As the incline steepens, $N$ decreases — eventually the block slides off." }
  ],
  ...
  "energy-work": [
    { f: "$W = \\vec F \\cdot \\vec d = Fd\\cos\\theta$", n: "WORK done by a force. ORIGIN: Coriolis (1829) defined work in this precise form, building on earlier intuitive uses by Carnot and Lazare. WHY THE DOT PRODUCT: only the component of force ALONG the motion contributes; force perpendicular to motion does zero work (which is why centripetal force does no work — it never speeds up the object). WORKED EXAMPLE: push a $50$ N crate $10$ m horizontally — work = $50 \\times 10 = 500$ J. Push it $10$ m at $60°$ above horizontal — work = $50 \\times 10 \\times \\cos 60° = 250$ J. Holding a heavy book still: zero displacement, zero work (even though your muscles get tired — that's biology, not physics)." },
    { f: "$KE = \\tfrac{1}{2} m v^2$", n: "KINETIC ENERGY — energy of motion. ORIGIN: Leibniz (~1686) called $mv^2$ the 'vis viva' (living force). The factor of $\\tfrac{1}{2}$ comes from integrating Newton's $F = ma$ over distance: $\\int F\\,dx = \\int ma\\,dx = \\int mv\\,dv = \\tfrac{1}{2}mv^2$. WHY $v^2$ AND NOT $v$: double the speed quadruples the KE — this is why high-speed crashes are dramatically worse than low-speed ones. A car at $60$ mph has four times the KE of one at $30$ mph. WORKED EXAMPLE: a $1500$ kg car at $20$ m/s has $KE = \\tfrac{1}{2}(1500)(400) = 300{,}000$ J — enough to lift the car ~$20$ m straight up." },
    { f: "$PE_g = mgh$", n: "GRAVITATIONAL POTENTIAL ENERGY near Earth's surface. ORIGIN: derived from $W = mg \\cdot h$ (work done against gravity to lift mass $m$ a height $h$). REFERENCE HEIGHT is arbitrary — you can call any height zero; only DIFFERENCES matter physically. WORKED EXAMPLE: lift a $5$ kg book $1.5$ m onto a shelf. PE change: $5 \\times 9.8 \\times 1.5 = 73.5$ J. Drop it back down — that PE converts to KE (just before impact: $v = \\sqrt{2gh} = \\sqrt{29.4} \\approx 5.4$ m/s). For large heights, this formula breaks (gravity varies with altitude); use $PE = -GMm/r$ instead." },
    { f: "$PE_s = \\tfrac{1}{2} k x^2$", n: "SPRING POTENTIAL ENERGY (also called ELASTIC PE). ORIGIN: integrate Hooke's law $F = -kx$ from $0$ to $x$ to find work done compressing the spring: $\\int_0^x kx'\\,dx' = \\tfrac{1}{2}kx^2$. WHY $x^2$: like KE, doubling displacement quadruples the energy stored. WORKED EXAMPLE: a spring with $k = 200$ N/m compressed by $0.1$ m stores $\\tfrac{1}{2}(200)(0.01) = 1$ J. Release it pushing a $0.05$ kg block: $1$ J = $\\tfrac{1}{2}mv^2$, so $v = \\sqrt{40} \\approx 6.3$ m/s. Pogo sticks, bullet cartridges, and trampolines all work this way." },
    { f: "$W_{\\text{net}} = \\Delta KE$", n: "WORK-ENERGY THEOREM. ORIGIN: a direct consequence of Newton's 2nd law integrated over distance. The NET work done on an object equals its CHANGE in kinetic energy. POWER: lets you skip force-by-force analysis and just compare initial and final KE. WORKED EXAMPLE: a $1000$ kg car decelerates from $30$ m/s to $0$ over $100$ m (skid stop). Change in KE: $0 - \\tfrac{1}{2}(1000)(900) = -450{,}000$ J. Net work done by friction: $-450{,}000$ J. Friction force: $W/d = 450{,}000/100 = 4500$ N. Skipped the entire $F = ma$ calculation." },
    { f: "$E_{\\text{total}} = KE + PE = $ const", n: "CONSERVATION OF MECHANICAL ENERGY. ORIGIN: the principle was developed across centuries — Leibniz noticed conservation of $mv^2$ in collisions; Lagrange and others formalized energy conservation in mechanics. The full law of energy conservation (including heat) came with Joule's 1843 experiments. APPLIES WHEN only conservative forces act (gravity, springs, electric, magnetic). FAILS when friction is present — energy is then converted to heat. WORKED EXAMPLE: drop a ball from height $h$. At the top: $KE = 0, PE = mgh$. At the bottom: $PE = 0$, $KE = mgh$, so $v = \\sqrt{2gh}$. Mass cancels — same speed for any object, as Galileo discovered." },
    { f: "$P = dW/dt = \\vec F \\cdot \\vec v$", n: "POWER — rate at which work is done. ORIGIN: James Watt introduced 'horsepower' (1 hp = 746 W) to market his steam engines in the 1780s. UNITS: 1 watt = 1 joule per second. The unit (and the surname Watt) honor him. WORKED EXAMPLE: a $100$ W lightbulb consumes $100$ J every second. Run it for an hour: $360{,}000$ J = 0.1 kWh of electrical energy. Lifting yourself ($70$ kg) up stairs ($3$ m) in $2$ s: power = $mgh/t = 70 \\cdot 9.8 \\cdot 3/2 \\approx 1029$ W — about $1.4$ hp, briefly!" }
  ],
